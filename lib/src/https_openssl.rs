...
pub enum ListenerError {
    ...
    #[error("failed to build openssl context")]
    BuildOpenSslError,
}
...
impl CertificateResolver for Listener {
    ...
    fn get_certificate(&self, fingerprint: &CertificateFingerprint) -> Option<CertificateAndKey> {
        let resolver = self
            .resolver
            .lock()
    ...
    fn add_certificate(
        ...
        let (fingerprint, certificate_and_key) = {
            ...
            let certificate_and_key = resolver
                .get_certificate(&fingerprint)
                .expect("to retrieve certificate and key has they just been inserted");

            (fingerprint, certificate_and_key)
        };
        ...
        let key = PKey::private_key_from_pem(certificate_and_key.key.as_bytes())
            .map_err(|err| ListenerError::PemParseError(err.to_string()))?;
        let chains = certificate_and_key
            .certificate_chain
            .iter()
            .filter_map(|chain| X509::from_pem(chain.as_bytes()).ok())
            .collect::<Vec<_>>();
        ...
        let (context, _) = match Self::create_context(
            versions,
            ...
            None => return Err(ListenerError::BuildOpenSslError),
    ...
    if configuration.add_listener(config, token).is_some() {
            ...
            .is_some()
        {
            ...
            let mut server = Server::new(
                ...
                backends,
                None,
                Some(HttpsProvider::Openssl(configuration)),
                None,
                server_config,
                None,
                false,
