...
impl CertificateResolver for GenericCertificateResolver {
    ...
    fn add_certificate(
        ...
        let new_names = self.certificate_names(&pem)?;
        let expiration = self
        ...
        if !should_insert {
            let diff: HashSet<&String> = new_names.difference(&certs_names).collect();
            if diff.is_empty() {
        ...
        self.certificates
            .insert(fingerprint.to_owned(), opts.certificate.to_owned());
...
impl CertificateResolverHelper for GenericCertificateResolver {
    ...
    fn certificate_names(&self, pem: &Pem) -> Result<HashSet<String>, Self::Error> {
        ...
        for name in x509.subject().iter_by_oid(&OID_X509_COMMON_NAME) {
            names.insert(name.attr_value.as_str().map(String::from).map_err(|err| {
            ...
            if !keys.is_empty() {
                ...
                if let Ok(keys) = parsed_key {
                    if !keys.is_empty() {
                        if let Ok(signing_key) = RSASigningKey::new(&keys[0]) {
                            let certified =
                                CertifiedKey::new(chain, Arc::new(Box::new(signing_key)));
                            return Some(certified);
                        } else {
